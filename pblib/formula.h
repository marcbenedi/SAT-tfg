...
typedef std::shared_ptr<FormulaClass> Formula; //MARC: Shared pointer -> amb contador de referències
...
//MARC: Per crear nous elements (així gestiona la memoria amb els punters)
static Formula newFalse() {
        return std::make_shared<FormulaClass>(0, 0);
}
static Formula newTrue() {
        return std::make_shared<FormulaClass>(0, 1);
}
static Formula newUndef() {
        return std::make_shared<FormulaClass>(0, 2);
}
...
inline Lit getLit(Formula const & f ) {
        assert(isAtom(f)); return ((f->data & 1) != 0 ?  -1 * (f->data >> 2) : (f->data >> 2));
}
inline Lit getId(Formula const & f) {
        assert(!isAtom(f)); return ((f->data & 1) != 0 ?  -1 * (f->data >> 2) : (f->data >> 2));
}
...
inline Formula OR (Formula const & f, Formula const & g) {
        return ~(AND(~f, ~g));
}
...
inline Formula LIT(Lit lit) {
        return FormulaClass::newLit(lit);
}



inline Formula true_branch(Formula const & f) {
        assert(isITE(f) || isMonotonicITE(f)); return f->input_nodes[1];
}
inline Formula false_branch(Formula const & f) {
        assert(isITE(f) || isMonotonicITE(f)); return f->input_nodes[2];
}
inline Formula selector(Formula const & f) {
        assert(isITE(f) || isMonotonicITE(f)); return f->input_nodes[0];
}
inline Formula left(Formula const & f) {
        assert(isAND(f) || isEquiv(f)); return f->input_nodes[0];
}
inline Formula right(Formula const & f) {
        assert(isAND(f) || isEquiv(f)); return f->input_nodes[1];
}
